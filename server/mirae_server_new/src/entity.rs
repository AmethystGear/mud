...
pub trait Entity {
    ...
    fn do_random_ability(
        ...
        if ability_names.len() > 0 {
            let rand_ability = self
                .abilities()
                .get(ability_names[rng.gen_range(0, ability_names.len())])
                .expect("bug")
                .clone();
            self.run_ability(opponent, battle_map, rand_ability, None, g)
        } else {
    ...
    fn try_random_move(
        ...
        if random_move < 0.1 {
            ...
            if let Some(item) = self.equipped().items().next() {
                let item = g
                    .items
                    .get(item)
                    .ok_or(anyhow!(format!("invalid item name! {:?}", item)))?;

                self.do_random_ability(opponent, battle_map, &item.abilities, g, rng)
            } else {
                Err(anyhow!("no item equipped"))
            }
        } else if random_move < 0.7 {
            // attempt to do a random inherent ability
            self.do_random_ability(opponent, battle_map, &self.abilities(), g, rng)
    ...
    fn do_random_move(&mut self, opponent: Option<Box<&mut dyn Entity>>, battle_map : &mut BattleMap, g: &GameData) {
    ...
    fn deal_damage(&mut self, damage_dealt: &HashMap<DmgType, f64>, g: &GameData) {
        let mut sum = 0.0;
        for (dmg, amount) in damage_dealt {
            if amount.abs() > f64::EPSILON {
                self.send_text(format!("you recieved {} {:?} damage\n", amount, dmg));
                sum += amount;
            }
        }
        if sum.abs() > f64::EPSILON {
            self.send_text(format!("you were dealt {} total damage.\n", sum));
            self.stats_mut().change_health(sum, g);
        }
    }
}
