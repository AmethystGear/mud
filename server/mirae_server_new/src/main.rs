...
use combat::{BattleMap, EntityType, ID};
...
use vector3::Vector3;
use websocket::{sync::{Server, Client}, OwnedMessage};
use world::World;
...
fn map(mut params: VecDeque<Literal>, world: &World, g: &GameData) -> Result<()> {
    ...
    let image = Image::new(&world, &g, &bounds, resolution as usize)?;
    save_img(image, &save_location)?;
    Ok(())
}

fn look(mut params: VecDeque<Literal>, world: &World, g: &GameData) -> Result<()> {
    let x;
    let y;
    let z;
    let width;
...
fn handle_server_commands(world: Arc<Mutex<World>>, g: &GameData) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    spawn(move || {
        loop {
            ...
            match res {
                    ...
                    match res {
                        ConnA::Init(s) => {
                            ...
                            if let Some(id) = id {
                                ...
                                players[id] =
                                    Some(Player::new(id, sender, &g_arc, &mut world.rng).unwrap());
                            } else {
                                let mut p_out = PlayerOut::new();
                ...
                Err(_) => { /*no input to handle, do any world logic */ }
            }
        }
    });
    ...
    loop {
        if let Err(res) = handle_server_commands(Arc::clone(&world), &g) {
            println!("{:?}\n", res);
        }
    }
}
...
fn handle_connection(stream: Client<TcpStream>, channel: Sender<ConnA>) {
    ...
    loop {
        ...
        channel.send(ConnA::Run((VecDeque::from_iter(params), id))).unwrap();
    }
