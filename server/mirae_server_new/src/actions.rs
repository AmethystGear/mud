    ...
    if let Ok(opponent) = data.battle_map.get_opponent(player_id) {
        if data.battle_map.turn(opponent)? {
            ...
            while both_stun {
                ...
                let battle_data = data.battle_map.get_battle_data(opp_id)?;
                let opp_stun = battle_data.combat_data(opp_id)?.stunned;
                let stun = battle_data.combat_data(player_id)?.stunned;
                both_stun = opp_stun && stun;
            }
            Ok(())
        }
    } else {
...
fn step(mut data: ActionData) -> Result<()> {
    ...
    player.send_text(format!("moved to: {:?}", player.loc()));
...
fn map(data: ActionData) -> Result<()> {
    ...
    let resolution = (2).min((data.world.blocks().dim.x() as usize) / (u8::MAX as usize));
    let img = Image::new(data.world, data.g, &bounds, resolution)?;
    let player = get_mut(data.players, data.player_id)?;
...
fn disp(data: ActionData) -> Result<()> {
    ...
    let img = Image::new(data.world, data.g, &bounds, 1)?;
    let player = get_mut(data.players, data.player_id)?;
    player.send_image(img);
    Ok(())
}
...
fn equip(mut data: ActionData) -> Result<()> {
    ...
    match (data.params.pop_front(), data.params.pop_front()) {
        (Some(Literal::String(s)), None) => match s.as_str() {
            "dequip" => {
                player.send_text("dequipping item...".into());
                player.dequip()
            }
            "equip" => {}
            _ => return Err(anyhow!(BAD_ARGS)),
...
fn battle(data: ActionData) -> Result<()> {
    ...
    if let Some(opp) = opponent_id {
        ...
        data.battle_map.init_battle(Box::new(player), Box::new(opponent), data.g)?;
        Ok(())
    } else {
