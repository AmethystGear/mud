    ...
    for var in vars {
        ...
        if stats::has_var(&buff, var.as_str()) {
            ...
            stats::set(
                &mut base,
                var.as_str(),
                stats::Value::Int(((curr as f64) * b) as i64),
            );
        }
    }
...
pub fn from(
    ...
    let defaults: Stats = stats::from(&mut scanner::from(CharStream::from_file(File::open(
        DEFAULT,
    )?)))?;
...
pub fn respawn(player: &mut Player, world: &World) -> Result<(), Box<dyn Error>> {
    ...
    set_posn(player, x, y)?;
    reset_to_base(player)?;
    let health = stats::get(
        &stats::get(player.data(), "buffed_stats")?.as_box()?,
        "health",
    )?
    .as_int()?;
...
pub fn change_stat(player: &mut Player, stat: &str, delta: i64) -> Result<(), Box<dyn Error>> {
    ...
    stats::set(
        &mut stats,
        stat,
        Value::Int(std::cmp::min(stat_max, std::cmp::max(0, stat_val + delta))),
    );
...
pub fn reset_to_base(player: &mut Player) -> Result<(), Box<dyn Error>> {
    ...
    stats::set(
        &mut base_stats,
        "health",
        Value::Int(std::cmp::min(health, max_health)),
    );
...
pub fn change_xp(player: &mut Player, delta: i64) -> Result<(), Box<dyn Error>> {
    ...
    stats::set(
        &mut player.data,
        "xp",
        Value::Int(std::cmp::max(0, xp + delta)),
    );
    return Ok(());
}
...
pub fn upgrade_stat(player: &mut Player, stat: &str) -> Result<(), Box<dyn Error>> {
    ...
    if xp_cost > xp(player)? {
        return Err(format!(
            "you need {} xp to upgrade this stat, but you only have {} xp.",
            xp_cost,
            xp(player)?
        )
        .into());
