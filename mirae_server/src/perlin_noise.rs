...
fn perlin_noise(width: u16, height: u16, octave: u8, white_noise: &Vec<f64>) -> Vec<f64> {
    ...
    for j in 0..(height as usize) {
        ...
        for i in 0..(width as usize) {
            ...
            let top = lerp(get(white_noise, width as usize, x1, y1), get(white_noise, width as usize, x2, y1), x_blend);
            let bottom = lerp(get(white_noise, width as usize, x1, y2), get(white_noise, width as usize, x2, y2), x_blend);
            set(&mut result, width as usize, i, j, lerp(top, bottom, y_blend));
        }
    }
    return result;
}
